...
class NewsletterController extends Controller
{
    ...
    public function subscribeAction(Request $request)
    {
        ...
        if ($this->getUser() && $this->getUser()->getEmail() !== $email) {
            ...
            $mailler->addLisntingNewwsLetter(
                $this->getUser() === true ? $this->getUser() : null,
                $this->getUser() === true ? null : $email
            );
    ...
    public function unsubscribeAction($email, Request $request)
    {
        ...
        if ($form->isSubmitted() && $form->isValid()) {
            ...
            $request->request->add(['status' => $mailer->isSuccess()]);
            return $this->redirectToRoute('newsletter_status', array('email' => $email));
        ...
        if ($device->isMobile() || $device->isTablet()) {
            return $this->render(
                '@Observation/Newsletter/Mobile/unsubscribe.html.twig',
                array('form' => $form->createView(), 'email' => $email)
            );
        } else {
            return $this->render(
                '@Observation/Newsletter/Desktop/unsubscribe.html.twig',
                array('form' => $form->createView(), 'email' => $email)
            );
        }
    }

    public function statusAction($email, Request $request)
    {
        return $this->render('@Observation/Newsletter/status.html.twig', array('email' => $email, 'status' => $request->request->get('status')));
    }
}
